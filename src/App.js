...
import AddingHistories from './components/AddHistory'
...
class App extends Component {
  ...
  render() {
    ...
    const newUpdateHistory = historyList.filter(eachItem =>
      eachItem.title.toLowerCase().includes(searchInput.toLowerCase()),
    )
    const checkHistory = () => {
      if (newUpdateHistory.length !== 0 || searchInput !== ' ') {
        return (
          <ul className="sectioncontainer">
            {newUpdateHistory.map(eachitem => (
              <AddingHistories
    ...
    return (
      <div>
        ...
        <div className="AllhistoryContainer">{checkHistory()}</div>
      </div>
    )
  }
}
